...
## 本地改动记录
1. 令牌名称长度上限调整为 128（原为 30），用于容纳设备号等长名称。
2. 增加阿里云 ASR 兼容转发：当 `/v1/audio/transcriptions` 请求的模型为 `qwen3-asr-*` 时，服务端会自动转换为 `/v1/chat/completions + input_audio` 请求，再回包为 transcription 结构，兼容 OpenClaw/FatClaw 的 provider 模式。
3. 该兼容分支会读取上传的音频文件并转成 data URI，转发时附带 `asr_options.enable_itn=false`。
4. 新增更明确的本地联调场景：FatClaw 的 TTS 通过 `/v1/audio/speech` 调用 OneAPI，ASR 通过兼容后的 `/v1/audio/transcriptions` 调用 OneAPI。

### FatClaw / OpenClaw 兼容说明

当前本地分支主要为 FatClaw 桌面端和 OpenClaw SDK 做了两类兼容：

1. TTS 兼容

- FatClaw 通过 OpenAI 兼容接口调用：
...
- OpenClaw provider 模式默认调用：

```bash
POST /v1/audio/transcriptions
```

- 但阿里云 `qwen3-asr-flash` 实际要求的是：

```bash
POST /v1/chat/completions
```

请求体使用 `messages[].content[].type=input_audio`。

因此本地分支在服务端做了一层自动转换，调用方无需改 SDK 协议。

### ASR 兼容请求示例

客户端照常发 transcription 请求：

```bash
curl -sS -X POST "http://127.0.0.1:3000/v1/audio/transcriptions" \
...
服务端会自动转成等价的 chat completions 请求，并最终返回：
...
1. 若 OneAPI 渠道侧配置了模型映射，请避免把 `qwen3-asr-flash` 再映射到 `whisper-1`，否则会绕过这层兼容逻辑。
2. 兼容转换分支当前主要面向 `qwen3-asr-*` 模型；普通 OpenAI transcription 模型仍按原始流程处理。
3. FatClaw 本地联调默认地址使用 `http://127.0.0.1:3000`，生产环境再由桌面端切换到正式域名。
...
1. 额度是什么？怎么计算的？One API 的额度计算有问题？
   + 文本类：额度 = 分组倍率 * 渠道倍率 * 模型倍率 * （提示 token 数 + 补全 token 数 * 补全倍率）
   + 模型倍率优先级：`model(channelType)` → `model` → 默认值（未命中时默认返回 `30`）。
   + 补全倍率优先级：`model(channelType)` → `model` → 规则默认（如 GPT3.5 为 1.33、GPT4 为 2 等）。
   + 渠道倍率：来自渠道配置（未配置时为 `1`）。
